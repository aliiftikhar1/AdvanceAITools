...
const PricingCardVTC = (props) => {
  ...
  return (
    <>
      <div
        className={`relative p-10 w-[345px] rounded-xl shadow-lg transform transition-transform duration-300 ease-in-out hover:scale-105 bg-white border-2 flex flex-col justify-between ${
      ...
      >
        ...
        <ul className="text-left space-y-4 mb-8">
          <li className="flex items-center text-gray-700">
            <span className="text-black mr-2"><SiTicktick/> </span> Credits: 
            
            {props.credits==999?(<> Unlimited</>):(<> {props.credits}</>)}
          </li>
          <li className="flex items-center text-gray-700">
          <span className="text-black mr-2"><SiTicktick/> </span> Length: {props.length}
          </li>
        </ul>
              ...
              <div className="w-1/2 p-8 flex flex-col justify-between">
                <div>
                  ...
                  <ul className="mb-6 space-y-2">
                    <li className="text-gray-700">Credits:  {props.credits == 999 ? (<>
                    Unlimited
                    </>):(<>
                    {props.credits}
                    </>)}</li>
                    <li className="text-gray-700">Length: {props.length}</li>
                  </ul>
